      ...
      if (face1 != null) {
        ...
        if (result.getFaceBiometrics() != null) {
          ...
          if (result.getLivenessResult().getLivenessStatus()) {
            String lvscore = result.getLivenessResult().getLivenessScore() + "";
            try {
...
//    }setEnableFaceMovement setEnableOralVerification

  private void openLivenessCamera() {
    LivenessCustomization livenessCustomization = new LivenessCustomization();

//    System.out.println("livenessConfig3: " );
    try {
      boolean enableFaceMovement = false;
      boolean enableOralVerification = false;
      if(livenessConfig.has("setEnableFaceMovement") ){
        enableFaceMovement = livenessConfig.getBoolean("setEnableFaceMovement");

      }
      if(livenessConfig.has("setEnableOralVerification") ){
        enableOralVerification = livenessConfig.getBoolean("setEnableOralVerification");

      }

      if(enableFaceMovement && enableOralVerification){
        livenessCustomization.livenessMode = LivenessMode.ALL;
      }else if(enableFaceMovement && !enableOralVerification){
        livenessCustomization.livenessMode = LivenessMode.FACE_3D;
      }else if(!enableFaceMovement && enableOralVerification){
        livenessCustomization.livenessMode = LivenessMode.VOICE;
      }else{
        livenessCustomization.livenessMode = LivenessMode.DEFAULT;
      }
    } catch (JSONException e) {
      e.printStackTrace();
    }
