...
//        print("\(self.goNativeArgs)")
      ...
      if configs["setIsDocEnable"] != nil {
          let val = (configs["setIsDocEnable"] as? Bool)!
          self.accuraCameraWrapper?.isApiCallingEnable(val)
      }else{
          self.accuraCameraWrapper?.isApiCallingEnable(false)
      }
    
      if configs["setDocUrl"] != nil {
          let val = (configs["setDocUrl"] as? String)!
              self.accuraCameraWrapper?.apiurl(val)
      }
      
    if configs["setProgressMessage"] != nil {
      let val = (configs["setProgressMessage"] as? String)
      self.accuraCameraWrapper?.setProgressDialogue(val)
    } else{
      self.accuraCameraWrapper?.setProgressDialogue("API Calling...")
    }
    
    if configs["setHideProgressDialogue"] != nil {
        let val = (configs["setHideProgressDialogue"] as? Bool)!
      self.accuraCameraWrapper?.hideProgressDialogue(val)
    }

      
      if configs["setApiTimer"] != nil {
          let val = (configs["setApiTimer"] as? Int32)!
          self.accuraCameraWrapper?.apiTimer(Float(val))
      }else{
        self.accuraCameraWrapper?.apiTimer(100.0)
      }
      
    if configs["setHeader"] != nil {
        let val = (configs["setHeader"] as? [String: String])!
        self.accuraCameraWrapper?.apiheader((val))
    }
